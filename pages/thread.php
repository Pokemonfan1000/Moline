...
if($loguserid)
{
	...
	if(CanMod($loguserid,$forum['id']) && IsAllowed("editThread", $tid))
	{
		...
		if($forum['id'] != Settings::get('hiddenTrashForum'))
			$links -> add(new PipeMenuLinkEntry(__("Trash"), "editthread", $tid, "action=delete&key=".$loguser['token'], "remove"));
		if($forum['id'] != Settings::get('trashForum'))
			$links -> add(new PipeMenuLinkEntry(__("Treasure"), "editthread", $tid, "action=trash&key=".$loguser['token'], "trash"));
	}
